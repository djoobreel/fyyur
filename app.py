...
def search_venues():
  ...
  venues = db.session.query(Venue).filter(Venue.name.like("%" + search_term +"%"))
  data = []
  for venue in venues:
    venue_data = {
    "id": venue.id,
...
def search_artists():
  search_term=request.form.get('search_term', '')
  artists = db.session.query(Artist).filter(Artist.name.like("%" + search_term +"%"))
  data = []
  for artist in artists:
    artist_data = {
    "id": artist.id,
...
def create_show_submission():
  ...
  if not artist:
    ...
    return render_template('forms/new_show.html', form=form)
  venue = db.session.query(Venue).get(int(venue_id))
  if not artist:
